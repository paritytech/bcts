...
function addVector(
  ...
  // Verify round-trip
  const { msg: decoded, index: decodedIndex, bytesRead } =
  ...
  if (decoded.payload.type !== payload.type) {
    throw new Error(
      `${name}: type mismatch ${decoded.payload.type} !== ${payload.type}`,
    );
  }
  if (bytesRead !== bytes.length) {
    throw new Error(
      `${name}: bytesRead mismatch ${bytesRead} !== ${bytes.length}`,
    );
  }
...
addVector(
  'hdr-basic',
  'Hdr message: epoch=1, index=0, chunk(idx=0, data=0x00*32)',
  1n,
  0,
  { type: 'hdr', chunk: makeChunk(0, 0x00) },
);

addVector(
  'ek-basic',
  'Ek message: epoch=1, index=1, chunk(idx=0, data=0x11*32)',
  1n,
  1,
  { type: 'ek', chunk: makeChunk(0, 0x11) },
);
...
addVector(
  'ct1-basic',
  'Ct1 message: epoch=1, index=3, chunk(idx=0, data=0x33*32)',
  1n,
  3,
  { type: 'ct1', chunk: makeChunk(0, 0x33) },
);

addVector(
  'ct2-basic',
  'Ct2 message: epoch=1, index=4, chunk(idx=0, data=0x44*32)',
  1n,
  4,
  { type: 'ct2', chunk: makeChunk(0, 0x44) },
);
...
addVector(
  'none-epoch128',
  'None with epoch=128 (2-byte LEB128: 0x80 0x01)',
  128n,
  0,
  { type: 'none' },
);
...
addVector(
  'none-epoch16384',
  'None with epoch=16384 (3-byte LEB128: 0x80 0x80 0x01)',
  16384n,
  0,
  { type: 'none' },
);
...
addVector(
  'none-index128',
  'None with index=128 (2-byte LEB128)',
  1n,
  128,
  { type: 'none' },
);
...
addVector(
  'none-index300',
  'None with index=300 (2-byte LEB128: 0xAC 0x02)',
  1n,
  300,
  { type: 'none' },
);
...
addVector(
  'none-epoch1',
  'None with epoch=1 (minimum valid epoch)',
  1n,
  0,
  { type: 'none' },
);
...
addVector(
  'ct1ack-epoch1',
  'Ct1Ack with epoch=1. No payload. Minimum valid epoch.',
  1n,
  0,
  { type: 'ct1Ack' },
);
...
const output = {
  _comment:
    'SPQR V1 wire-format test vectors. Generated by TypeScript implementation.',
  _format:
