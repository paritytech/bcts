...
export async function processPreKeyMessage(
  ...
  if (
    message.kyberPreKeyId !== undefined &&
    message.kyberCiphertext &&
    kyberPreKeyStore
  ) {
    const kyberPreKey = await kyberPreKeyStore.loadKyberPreKey(
      message.kyberPreKeyId,
    );
    const { ciphertext: rawCiphertext } = kemDeserializeCiphertext(message.kyberCiphertext);
    kyberSharedSecret = kemDecapsulate(
      rawCiphertext,
      kyberPreKey.keyPair.secretKey,
    );
  }
  ...
  newSession.setLocalRegistrationId(
