...
const X3DH_INFO = new TextEncoder().encode(
...
export function initializeBobSession(
  ...
  secrets.push(
    x25519RawAgreement(
      params.ourSignedPreKeyPair.privateKey,
      params.theirIdentityKey.publicKey,
    ),
  );
  ...
  secrets.push(
    x25519RawAgreement(
      params.ourIdentityKeyPair.privateKey,
      params.theirBaseKey,
    ),
  );
  ...
  if (params.ourOneTimePreKeyPair) {
    secrets.push(
      x25519RawAgreement(
        params.ourOneTimePreKeyPair.privateKey,
        params.theirBaseKey,
      ),
    );
  }
