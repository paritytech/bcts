  ...
  switch (msgType) {
    ...
    default:
      throw new InvalidSealedSenderMessageError(
        `Unknown ciphertext message type: ${msgType}`,
      );
  }
}

function protoTypeToCiphertextMessageType(
  protoType: number,
  ...
  switch (protoType) {
    ...
    default:
      throw new InvalidSealedSenderMessageError(
        `Unknown proto message type: ${protoType}`,
      );
  }
}
...
export class UnidentifiedSenderMessageContent {
  ...
  static deserialize(
    ...
    if (protoType === undefined || !senderCertBytes || !content) {
      throw new InvalidSealedSenderMessageError(
        "Invalid USMC: missing required fields",
      );
    }
    ...
    if (contentHintRaw !== undefined) {
      if (
        contentHintRaw === ContentHint.Resendable ||
        contentHintRaw === ContentHint.Implicit
      ) {
...
function v2ComputeAuthenticationTag(
  ...
  const agreement = x25519RawAgreement(
    ourIdentity.privateKey,
    theirIdentityPublicKey,
  );

  const ourPubSerialized = serializePublicKey(
    ourIdentity.identityKey.publicKey,
  );
...
export function sealedSenderEncrypt(
  ...
  const ephemeralPubSerialized = serializePublicKey(
    ephemeralKeyPair.publicKey,
  );

  const ephSecret = x25519RawAgreement(
  ...
  const senderPubSerialized = serializePublicKey(
    senderIdentityKeyPair.identityKey.publicKey,
  );
  ...
  const staticKeys = deriveStaticKeysFromSecret(
    staticSecret,
    ephKeys.chainKey,
    encryptedStatic,
  );
  ...
  const encryptedMessage = aes256CtrHmacEncrypt(
    usmc,
  ...
  return concatBytes(
    new Uint8Array([SEALED_SENDER_V1_FULL_VERSION]),
    proto,
  );
}
...
export function sealedSenderEncryptV2(
  ...
  const encryptedMessage = aes256GcmSivEncrypt(
    k,
    zeroNonce,
    usmc.serialize(),
  );
...
function decryptV1ToUsmc(
  ...
  const recipientPubSerialized = serializePublicKey(
  ...
  try {
    senderPubSerialized = aes256CtrHmacDecrypt(
      encryptedStatic,
      ephKeys.cipherKey,
      ephKeys.macKey,
    );
  ...
  try {
    messageBytes = aes256CtrHmacDecrypt(
      encryptedMessage,
      staticKeys.cipherKey,
      staticKeys.macKey,
    );
  ...
  if (
    !constantTimeEqual(
      senderPubSerialized,
      serializePublicKey(usmc.senderCertificate.identityKey),
    )
  ) {
    throw new InvalidSealedSenderMessageError(
      "sender certificate key does not match message key",
    );
  }
...
function decryptV2ToUsmc(
  ...
  let offset = 1;
  const encryptedMessageKey = ciphertext.slice(
    offset,
...
export class SealedSenderMultiRecipientMessage {
  ...
  static parse(
    ...
    for (let i = 0; i < recipientCount; i++) {
      ...
      if (version === SEALED_SENDER_V2_UUID_FULL_VERSION) {
        if (offset + 16 > data.length) {
          throw new InvalidSealedSenderMessageError(
            "Message truncated at service ID",
          );
        }
      ...
      for (;;) {
        if (offset >= data.length) {
          throw new InvalidSealedSenderMessageError(
            "Message truncated at device list",
          );
        }
        ...
        if (offset + 2 > data.length) {
          throw new InvalidSealedSenderMessageError(
            "Message truncated at registration ID",
          );
        }
      ...
      if (hasDevices && devices.length > 0) {
        ...
        if (offset + keyMaterialLen > data.length) {
          throw new InvalidSealedSenderMessageError(
            "Message truncated at key material",
          );
        }
      ...
      if (existing) {
        if (existing.devices.length === 0 || devices.length === 0) {
          throw new InvalidSealedSenderMessageError(
            "recipient redundantly encoded as empty",
          );
        }
    ...
    if (offset + PUBLIC_KEY_LEN > data.length) {
      throw new InvalidSealedSenderMessageError(
        "Message truncated at ephemeral public key",
      );
    }

    const sharedBytes = data.slice(offset);

    return new SealedSenderMultiRecipientMessage(
      version,
      recipients,
      sharedBytes,
    );
   ...
   */
  recipientsByServiceIdString(): Map<
    string,
   ...
   */
  messageForRecipient(
    recipient: SealedSenderV2SentRecipient,
  ): Uint8Array {
    return concatBytes(
      new Uint8Array([SEALED_SENDER_V2_UUID_FULL_VERSION]),
      recipient.cBytes,
 ...
 */
export function serviceIdFromUuid(
  uuid: string,
  type: number = 0x01,
): Uint8Array {
