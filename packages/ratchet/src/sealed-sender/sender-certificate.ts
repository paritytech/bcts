...
export class SenderCertificate {
  ...
  static deserialize(
    ...
    if (serverCertBytes) {
      ...
      throw new InvalidSealedSenderMessageError("Invalid sender certificate inner: no server certificate");
    }

    const senderE164 = senderE164Bytes ? new TextDecoder().decode(senderE164Bytes) : undefined;
    const identityKey = identityKeyBytes.length === 33 && identityKeyBytes[0] === 0x05
      ? identityKeyBytes.slice(1) : identityKeyBytes;
