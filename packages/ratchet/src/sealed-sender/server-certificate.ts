...
let knownCertsMap: Map<
  number,
  { trustRoot: Uint8Array; cert: ServerCertificate }
> | null = null;

function getKnownCertsMap(): Map<
  number,
  { trustRoot: Uint8Array; cert: ServerCertificate }
> {
 ...
 */
export function lookupKnownServerCertificate(
  id: number,
): ServerCertificate {
 ...
 */
export function getKnownCertificateTrustRoot(
  id: number,
  ...
  static deserialize(data: Uint8Array): ServerCertificate {
    ...
    try {
      ...
      throw new InvalidSealedSenderMessageError(
        "Invalid server certificate encoding",
      );
    }
    ...
    try {
      ...
      throw new InvalidSealedSenderMessageError(
        "Invalid server certificate inner encoding",
      );
    }
    ...
    if (keyId === undefined || !keyBytes) {
      throw new InvalidSealedSenderMessageError(
        "Invalid server certificate inner",
      );
    }

    const key =
      keyBytes.length === 33 && keyBytes[0] === 0x05
        ? keyBytes.slice(1)
        : keyBytes;
