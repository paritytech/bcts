...
async function messageEncryptImpl(
  ...
  const pqRatchetData = pqSendResult.message.length > 0
    ? pqSendResult.message
    : undefined;
  ...
  if (!theirIdentityKey) {
    throw new InvalidSessionError(
      `No remote identity key for ${remoteAddress}`,
    );
  }
...
function decryptMessageWithRecord(
  ...
  if (record.sessionState()) {
    ...
    try {
      const plaintext = decryptMessageWithState(
        currentState,
        ciphertext,
        rng,
      );
