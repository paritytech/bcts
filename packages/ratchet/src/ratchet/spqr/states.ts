    ...
    switch (state.type) {
      ...
      case "KeysUnsampled": {
        if (msg.epoch > state.epoch) {
          throw new SpqrStateError(
            `Epoch out of range: ${msg.epoch}`,
            "EPOCH_OUT_OF_RANGE",
          );
        }
        return state;
      ...
      case "EkSentCt1Received": {
        ...
        if (msg.payload.type === MessageType.Ct2) {
          ...
          const mac = ct2Data.slice(
            mlkem.CIPHERTEXT2_SIZE,
            mlkem.CIPHERTEXT2_SIZE + MAC_SIZE,
          );
      ...
      case "Ct1Sampled": {
        ...
        const ct1Ack =
          isEkCt1Ack || msg.payload.type === MessageType.Ct1Ack;
        ...
        if (isEk || isEkCt1Ack) {
          ...
          if (ct1Ack) {
            ...
            const derivedSecret = deriveSckaKey(
      ...
      case "EkReceivedCt1Sampled": {
        ...
        if (
          msg.payload.type === MessageType.Ct1Ack ||
          msg.payload.type === MessageType.EkCt1Ack
        ) {
      ...
      case "Ct1Acknowledged": {
        ...
        const hasEk =
          msg.payload.type === MessageType.Ek ||
          msg.payload.type === MessageType.EkCt1Ack;
