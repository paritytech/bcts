...
export class Authenticator {
  ...
  update(epoch: number, key: Uint8Array): void {
    ...
    const infoStr = new TextEncoder().encode(
      "Signal_PQCKA_V1_MLKEM768:Authenticator Update",
    );
  ...
  macCt(epoch: number, ct: Uint8Array): Uint8Array {
    const prefix = new TextEncoder().encode(
      "Signal_PQCKA_V1_MLKEM768:ciphertext",
    );
    const epochBe8 = epochToBeBytes(epoch);
  ...
  macHdr(epoch: number, hdr: Uint8Array): Uint8Array {
    const prefix = new TextEncoder().encode(
      "Signal_PQCKA_V1_MLKEM768:ekheader",
    );
    const epochBe8 = epochToBeBytes(epoch);
  ...
  static deserialize(data: {
    ...
    return new Authenticator(
      Uint8Array.from(data.rootKey),
      Uint8Array.from(data.macKey),
    );
  }
