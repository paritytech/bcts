...
function getFingerprint(
  ...
  // Round 0
  let hash = sha512(
    concatBytes(fingerprintVersion, keyBytes, stableId, keyBytes),
  );
...
function getEncodedString(fprint: Uint8Array): string {
  if (fprint.length < 30) {
    throw new FingerprintParsingError(
...
export class ScannableFingerprint {
  ...
  constructor(
    version: number,
    localFprint: Uint8Array,
    remoteFprint: Uint8Array,
  ) {
  ...
  compare(otherSerialized: Uint8Array): boolean {
    ...
    if (other.version !== this.version) {
      throw new FingerprintVersionMismatchError(
        other.version,
        this.version,
      );
    }
    ...
    const same1 = constantTimeEqual(
      other.localFingerprint,
      this.remoteFingerprint,
    );
    const same2 = constantTimeEqual(
      other.remoteFingerprint,
...
function parseLogicalFingerprint(data: Uint8Array): Uint8Array {
  ...
  throw new FingerprintParsingError(
    "Missing content in LogicalFingerprint",
  );
}
...
export class Fingerprint {
  ...
  static create(
    ...
    const localFprint = getFingerprint(
      iterations,
      localStableId,
      localIdentityKey,
    );
    const remoteFprint = getFingerprint(
      iterations,
      remoteStableId,
      remoteIdentityKey,
    );
