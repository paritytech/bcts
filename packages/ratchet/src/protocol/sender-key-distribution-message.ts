...
export class SenderKeyDistributionMessage {
  ...
  static deserialize(data: Uint8Array): SenderKeyDistributionMessage {
    if (data.length < 2) {
      throw new InvalidMessageError(`SenderKeyDistributionMessage too short: ${data.length}`);
    }
    ...
    if (proto.signingKey.length === 33 && proto.signingKey[0] === 0x05) {
      ...
      throw new InvalidMessageError(`Invalid signing key length: ${proto.signingKey.length}`);
    }
