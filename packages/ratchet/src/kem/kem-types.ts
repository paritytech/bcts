 ...
 */
export function kemDeserializePublicKey(data: Uint8Array): { kemType: KemType; publicKey: Uint8Array } {
  if (data.length < 2) throw new Error("KEM public key too short");
  const kemType = data[0] as KemType;
  if (kemType !== KemType.Kyber768 && kemType !== KemType.Kyber1024 && kemType !== KemType.MLKEM1024) {
 ...
 */
export function kemDeserializeCiphertext(data: Uint8Array): { kemType: KemType; ciphertext: Uint8Array } {
...
function kemTypeName(kemType: KemType): string {
  switch (kemType) {
    case KemType.Kyber768: return "Kyber768";
    case KemType.Kyber1024: return "Kyber1024";
    case KemType.MLKEM1024: return "MLKEM1024";
  }
}
