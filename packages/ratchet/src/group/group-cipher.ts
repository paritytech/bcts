...
export async function createSenderKeyDistributionMessage(
  ...
  if (!recordBytes) {
    ...
    const chainId = (crypto.getRandomValues(new Uint8Array(4)).reduce(
      (acc, b, i) => acc | (b << (i * 8)), 0,
...
export async function processSenderKeyDistributionMessage(
  ...
  const record = recordBytes
    ? SenderKeyRecord.deserialize(recordBytes)
    : new SenderKeyRecord();
...
function getSenderKey(
  ...
  if (jump > MAX_FORWARD_JUMPS) {
    throw new InvalidMessageError(
      `SenderKey message from too far into the future: ${jump}`,
    );
  }
