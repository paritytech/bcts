...
describe("Group Cipher", () => {
  ...
  it("rejects duplicate messages", async () => {
    ...
    await expect(groupDecrypt(skm.serialized, bobStore, aliceAddress)).rejects.toThrow(
  ...
  it("rejects unknown distribution ID", async () => {
    ...
    await expect(groupDecrypt(fakeBytes, bobStore, aliceAddress)).rejects.toThrow();
  });
});
