...
describe("IdentityKey", () => {
  ...
  it("should sign and verify", () => {
    ...
    expect(
      idkp.identityKey.verifySignature(message, signature),
    ).toBe(true);
  });
  ...
  it("should reject invalid signature", () => {
    ...
    badSig[0] ^= 0xff;
    expect(
      idkp.identityKey.verifySignature(message, badSig),
    ).toBe(false);
  });
});
...
describe("Alternate Identity Signatures (PNI)", () => {
  ...
  it("should create and verify alternate identity signature (class API)", () => {
    ...
    expect(
      secondary.identityKey.verifyAlternateIdentity(primary.identityKey, signature),
    ).toBe(true);
  });
  ...
  it("should not be symmetric", () => {
    ...
    expect(
      primary.identityKey.verifyAlternateIdentity(secondary.identityKey, signature),
    ).toBe(false);
  });
  ...
  it("should reject wrong other identity key", () => {
    ...
    expect(
      secondary.identityKey.verifyAlternateIdentity(unrelated.identityKey, signature),
    ).toBe(false);
  });
  ...
  it("should reject wrong signer identity key", () => {
    ...
    expect(
      unrelated.identityKey.verifyAlternateIdentity(primary.identityKey, signature),
    ).toBe(false);
  });
  ...
  it("should produce different signatures for same inputs (random nonce)", () => {
    ...
    expect(
      secondary.identityKey.verifyAlternateIdentity(primary.identityKey, sig1),
    ).toBe(true);
    expect(
      secondary.identityKey.verifyAlternateIdentity(primary.identityKey, sig2),
    ).toBe(true);
  });
