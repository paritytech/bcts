...
describe("X25519 (RFC 7748)", () => {
  ...
  it("Alice computes shared secret with Bob public key", () => {
    const shared = x25519RawAgreement(
      hexToBytes(alicePrivate),
      hexToBytes(bobPublic),
    );
  ...
  it("rejects all-zero public key (low-order point)", () => {
    ...
    expect(() =>
      x25519RawAgreement(hexToBytes(alicePrivate), zeroKey),
    ).toThrow("low-order");
  });
  ...
  it("rejects identity point (0x01 followed by zeros)", () => {
    ...
    identityKey[0] = 1;
    expect(() =>
      x25519RawAgreement(hexToBytes(alicePrivate), identityKey),
    ).toThrow("low-order");
  });
});
...
describe("ChainKey Derivation", () => {
  ...
  it("message key seed is deterministic", () => {
    ...
    expect(bytesToHex(ck1.messageKeySeed())).toBe(
      bytesToHex(ck2.messageKeySeed()),
    );
  });
  ...
  it("next chain key is deterministic", () => {
    const ck1 = new ChainKey(seed, 0);
    const ck2 = new ChainKey(seed, 0);
    expect(bytesToHex(ck1.nextChainKey().key)).toBe(
      bytesToHex(ck2.nextChainKey().key),
    );
  });
...
describe("RootKey Ratchet Step", () => {
  ...
  it("matches manual HKDF derivation", () => {
    ...
    expect(bytesToHex(newRootKey.key)).toBe(
      bytesToHex(derived.slice(0, 32)),
    );
    expect(bytesToHex(newChainKey.key)).toBe(
  ...
  it("different DH keys produce different ratchet outputs", () => {
    ...
    const kp2 = KeyPair.fromPrivateKey(hexToBytes(
      "5dab087e624a8a4b79e17f8b83800ee66f3bb1292618b6fd1c2f8b27ff88e0eb",
    ));
...
describe("XEdDSA", () => {
  ...
  it("verify fails when message is modified", () => {
    const sig = xeddsaSign(privateKey, message);
    const alteredMessage = new TextEncoder().encode(
      "XEdDSA cross-vector test messagF",
    );
