...
describe("Authenticator", () => {
  ...
  it("should reject incorrect MAC", () => {
    ...
    expect(() =>
      auth.verifyCt(1, new Uint8Array([1, 2, 3]), badMac),
    ).toThrow("Ciphertext MAC is invalid");
  });
...
describe("Message Format", () => {
  ...
  it("should round-trip all message types with data", () => {
    ...
    for (const msgType of types) {
      ...
      const msg: SpqrMessage = { epoch: 3, payload: { type: msgType, data } as SpqrMessage["payload"] };
