...
import {
  messageEncrypt,
  messageDecrypt,
} from "../src/session/session-cipher.js";
...
describe("Fuzz: Random message sequence", () => {
  it(
    "should correctly encrypt/decrypt 50 random-direction messages",
    async () => {
...
describe("Fuzz: Random out-of-order delivery", () => {
  it(
    "should decrypt 20 messages delivered in random shuffled order",
    async () => {
      const { aliceStore, bobStore, aliceAddress, bobAddress, rng } =
        await setupSession();
...
describe("Fuzz: Malformed protobuf — SignalMessage", () => {
  it(
    "should handle 100 random byte arrays without crashing",
    async () => {
      for (let i = 0; i < 100; i++) {
  ...
  it("should handle edge-case byte arrays", () => {
    ...
    // All 0xFF
    expect(() =>
      SignalMessage.deserialize(new Uint8Array(100).fill(0xff)),
    ).toThrow();
...
describe("Fuzz: Malformed protobuf — SessionState", () => {
  ...
  it("should handle edge-case byte arrays", () => {
    ...
    // All 0xFF
    expect(() =>
      SessionState.deserialize(new Uint8Array(50).fill(0xff)),
    ).toThrow();
  });
});
...
describe("Fuzz: Malformed protobuf — SessionRecord", () => {
  ...
  it("should handle edge-case byte arrays", () => {
    ...
    // All 0xFF
    expect(() =>
      SessionRecord.deserialize(new Uint8Array(100).fill(0xff)),
    ).toThrow();

    // Valid-looking length prefix but truncated
    expect(() =>
      SessionRecord.deserialize(new Uint8Array([0x0a, 0x80, 0x01])),
    ).toThrow();
  });
});
...
describe("Fuzz: Rapid chain advancement", () => {
  it(
    "should handle 100 messages encrypted then decrypted in order",
    async () => {
      const { aliceStore, bobStore, aliceAddress, bobAddress, rng } =
        await setupSession();
...
describe("Fuzz: Group cipher random sequence", () => {
  it(
    "should handle 30 random messages from 3 senders",
    async () => {
      const rng = createTestRng();
