...
describe("IncrementalMac", () => {
  it("throws on zero chunk size", () => {
    expect(() => new IncrementalMac(TEST_KEY, 0)).toThrow(
      "chunk size must be positive",
    );
  });

  it("throws on negative chunk size", () => {
    expect(() => new IncrementalMac(TEST_KEY, -1)).toThrow(
      "chunk size must be positive",
    );
  });
...
describe("IncrementalMacValidator", () => {
  it("validates correct MACs", () => {
    const input = new TextEncoder().encode(
      "this is a simple test input string",
    );
    ...
    // Validate
    const validator = new IncrementalMacValidator(
      TEST_KEY,
      TEST_CHUNK_SIZE,
      expectedMacs,
    );
