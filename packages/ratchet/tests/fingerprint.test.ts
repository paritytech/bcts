...
const DISPLAYABLE_FINGERPRINT_V1 =
...
describe("Fingerprint", () => {
  ...
  describe("version 1 test vectors", () => {
    ...
    it("produces correct scannable fingerprint for Alice", () => {
      ...
      expect(bytesToHex(aFprint.scannable.serialize())).toBe(
    ...
    it("produces correct scannable fingerprint for Bob", () => {
      ...
      expect(bytesToHex(bFprint.scannable.serialize())).toBe(
    ...
    it("scannable fingerprints compare correctly", () => {
      ...
      expect(aFprint.scannable.compare(bFprint.scannable.serialize())).toBe(
        true,
      );
      expect(bFprint.scannable.compare(aFprint.scannable.serialize())).toBe(
        true,
      );
      // Self-compare should fail (local/remote are swapped)
      expect(aFprint.scannable.compare(aFprint.scannable.serialize())).toBe(
        false,
      );
    });
  ...
  describe("scannable fingerprint serialization", () => {
    ...
    it("round-trip preserves compare behavior", () => {
      ...
      expect(bytesToHex(aDeserialized.serialize())).toBe(
        bytesToHex(aSerialized),
      );
  ...
  describe("version mismatch", () => {
    it("throws on version mismatch during compare", () => {
      ...
      expect(() =>
        v1Fprint.scannable.compare(v2Fprint.scannable.serialize()),
