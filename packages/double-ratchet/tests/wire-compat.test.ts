...
function parseRawProto(data: Uint8Array): RawProtoField[] {
  ...
  while (offset < data.length) {
    ...
    switch (wireType) {
      ...
      default:
        throw new Error(`Unsupported wire type ${wireType} at offset ${offset - 1}`);
    }
  }
...
describe("Task 9.1: Raw Protobuf Parser", () => {
  ...
  it("parses multiple fields in order", () => {
    ...
    const data = new Uint8Array([
      0x08,
      42, // field 1, varint, value=42
      0x12,
      0x01,
      0xff, // field 2, LEN, 1 byte
      0x18,
      0x00, // field 3, varint, value=0
    ]);
    const fields = parseRawProto(data);
  ...
  describe("known byte sequence verification", () => {
    it("manually constructed bytes decode correctly", () => {
      ...
      const manual = new Uint8Array([
        ...
        0x02,
        0xab,
        0xcd, // field 1: bytes [0xAB, 0xCD]
        0x10,
        0x96,
        0x01, // field 2: varint 150
        0x18,
        0x05, // field 3: varint 5
        0x22,
        0x03,
        0xde,
        0xad,
        0xbe, // field 4: bytes [0xDE, 0xAD, 0xBE]
      ]);
    ...
    it("our encoder produces identical bytes to manual construction", () => {
      ...
      const expected = new Uint8Array([
        0x0a, 0x02, 0xab, 0xcd, 0x10, 0x96, 0x01, 0x18, 0x05, 0x22, 0x03, 0xde, 0xad, 0xbe,
      ]);
...
describe("Task 9.3: PreKeySignalMessage wire format", () => {
  ...
  describe("manually constructed PreKeySignalMessage bytes", () => {
    it("decodes manually crafted bytes matching proto field numbers", () => {
      ...
      const manual = new Uint8Array([
        0x08,
        0x01, // field 1: pre_key_id=1
        0x12,
        0x02,
        0xaa,
        0xbb, // field 2: base_key
        0x1a,
        0x02,
        0xcc,
        0xdd, // field 3: identity_key
        0x22,
        0x01,
        0xee, // field 4: message
        0x28,
        0xe7,
        0x07, // field 5: registration_id=999
        0x30,
        0x2a, // field 6: signed_pre_key_id=42
      ]);
      ...
      const original = {
        ...
        senderChain: {
          ...
          messageKeys: [
            ...
            {
              index: 1,
              cipherKey: filler(32, 0x80),
              macKey: filler(32, 0x81),
              iv: filler(16, 0x82),
            },
          ],
        },
      ...
      senderKeyStates: [
        {
          ...
          senderMessageKeys: [{ iteration: 0, seed: filler(32, 0xdd) }],
          messageVersion: 3,
        },
      ],
    };
...
describe("Task 9.3: Fingerprint format", () => {
  ...
  it("CombinedFingerprints fields match fingerprint.proto", () => {
    ...
    expect(bytesToHex(localNested[0].value as Uint8Array)).toBe(bytesToHex(localContent));
  });
...
describe("Task 9.4: Cross-proto field number map", () => {
  ...
  const logicalFingerprintFields = [{ name: "content", field: 1, wire: WIRE_LEN }];
...
describe("Task 9.4: Key derivation vectors", () => {
  describe("Chain key HMAC seeds", () => {
    ...
    it("message key seed and next chain key are different", () => {
      ...
      const ck = new ChainKey(chainKeyBytes, 0);
      expect(bytesToHex(ck.messageKeySeed())).not.toBe(bytesToHex(ck.nextChainKey().key));
    });
  });
...
describe("Task 9.4: Deterministic test vectors", () => {
  ...
  describe("known ChainKey encoding", () => {
    it("produces exact bytes", () => {
      ...
      const expected = new Uint8Array([0x08, 0x07, 0x12, 0x04, 0xff, 0xff, 0xff, 0xff]);
      expect(bytesToHex(encoded)).toBe(bytesToHex(expected));
    });
  });
  ...
  describe("known MessageKey encoding (seed variant)", () => {
    it("produces exact bytes", () => {
      ...
      const expected = new Uint8Array([0x08, 0x00, 0x2a, 0x03, 0xab, 0xab, 0xab]);
      expect(bytesToHex(encoded)).toBe(bytesToHex(expected));
    });
  });
  ...
  describe("known PendingPreKey encoding", () => {
    it("produces exact bytes", () => {
      ...
      const expected = new Uint8Array([0x08, 0x01, 0x12, 0x02, 0x99, 0x99, 0x18, 0x02]);
      expect(bytesToHex(encoded)).toBe(bytesToHex(expected));
    });
  });
});
...
describe("Task 9.4: Wire-level cross-verification", () => {
  ...
  it("CombinedFingerprints encode -> raw parse -> nested LogicalFingerprint", () => {
    ...
    expect(bytesToHex(remoteNested[0].value as Uint8Array)).toBe(bytesToHex(remoteContent));
  });
});
